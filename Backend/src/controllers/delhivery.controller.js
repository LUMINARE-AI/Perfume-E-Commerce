...
export const generateShippingLabel = async (req, res) => {
  try {
    ...
    if (result.success) {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=label-${waybill}.pdf`
      );
...
export const downloadDocument = async (req, res) => {
  try {
    ...
    if (!waybill) {
      return res
        .status(400)
        .json({ success: false, message: "Waybill number is required" });
    }

    const result = await delhiveryService.downloadDocument(waybill, type);

    if (result.success) {
      //  CASE 1: JSON (Delhivery link wala case)
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=${type}-${waybill}.pdf`
      );
