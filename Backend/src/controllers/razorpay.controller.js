  ...
  if (orderId) {
    ...
    if (order && !order.delivery?.awb) {
      try {
        ...
        order.delivery = awb
          ? { provider: "delhivery", awb, status: "pending", trackingUrl: `https://www.delhivery.com/track-v2/package/${awb}` }
          : { provider: "delhivery", status: "pending", error: "Shipment creation failed" };
